...
export default async function ClientDetail({
  ...
  const [{ data: sub }, { data: brand }, { data: batches }, { data: invoices }, { data: activity }] =
    await Promise.all([
      supabase
        .from("subscriptions")
        .select("status, cycle_key, rate_card_key, current_period_end, delivery_hold, stripe_subscription_id, plans(name, key)")
        .eq("org_id", id)
        .maybeSingle(),
