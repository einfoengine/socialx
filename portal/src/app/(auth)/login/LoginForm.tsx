...
export default function LoginForm({
  ...
  next: string;
  error?: string;
  /** The site this host serves, or null on a host that serves no single site. */
    ...
    <main className="min-h-screen flex items-center justify-center bg-[#F4F2EF] dark:bg-[#050508] px-6 transition-colors duration-300">
      <div className="w-full max-w-[420px]">
        ...
        {/* Two ways out for someone who cannot sign in: buy, or go back to the
            site they came from. Both point at the site this host serves, and both
            disappear when there is no such site, because sending a visitor to one
            customer's website from a screen that could belong to any of them is
            worse than sending them nowhere. The divider is an element rather than
            a middot, which the copy rules rule out. */}
        {brand && (brand.checkoutUrl || brand.websiteUrl) && (
          <div className="mt-6 flex items-center justify-center gap-3 text-xs text-gray-500 dark:text-gray-500">
            {brand.checkoutUrl && (
              <span>
                Not a client yet?{" "}
                <a href={brand.checkoutUrl} className="text-[#2B50DC] dark:text-[#5B8DEF]">
                  See the plans
                </a>
              </span>
            )}
            {brand.checkoutUrl && brand.websiteUrl && (
              <span aria-hidden="true" className="h-3 w-px shrink-0 bg-black/15 dark:bg-white/15" />
            )}
            {brand.websiteUrl && (
              <a href={brand.websiteUrl} className="text-[#2B50DC] dark:text-[#5B8DEF]">
                Back to {brand.wordmark}
              </a>
            )}
          </div>
        )}
