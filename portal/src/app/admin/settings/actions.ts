...
function readValue(def: SettingDef, raw: FormDataEntryValue | null): unknown | { error: string } {
  switch (def.kind) {
    ...
    case "choice": {
      ...
      /* Validated against the declaration rather than trusted, because a select
         is only a constraint in a browser and this action is reachable without
         one. An unknown value is refused rather than coerced to the fallback:
         silently storing something other than what was submitted is how a
         policy screen lies about the policy in force. */
      if (!def.options?.some((o) => o.value === value)) {
        return { error: `${def.label} was set to something this setting does not offer.` };
      }
      return value;
    }
    case "origins": {
      ...
      for (const line of lines) {
        ...
        if (!origin) {
          return { error: `"${line}" is not a domain. Write it like https://example.com.` };
        }
...
export async function saveRateCardsAction(
  ...
  /* Pricing is rendered by every integrated website and on every checkout link, so the
