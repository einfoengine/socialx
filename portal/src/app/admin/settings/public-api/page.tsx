 ...
 *
 * Two things narrow that answer and they sit at different levels, which is why
 * they are on different screens. The master switch above is platform-wide: off
 * means every request needs a key, on every site at once. The list below belongs
 * to one site, because publishing is a decision about one website's copy.
 *
 * There is no origin list here any more. Which browsers may read a site's public
 * content is a property of that site's verified domains, so it moved to the site
 * record where the rest of that site's reach lives.
 */
export default async function PublicApiPage() {
  ...
  const [settings, ctx, supabase] = await Promise.all([
    readSettings(),
    adminSiteContext(),
    createClient(),
  ]);
    ...
    <div>
      ...
      {migrationMissing ? (
        ...
        <>
          <Panel
            title="Reach"
            sub="Platform-wide and absolute: with this off, every request needs a key, on every site, whatever is marked public below."
          >
            <SettingsForm
              group="API"
          ...
          >
            {site ? (
              <PublicEntries siteId={site.id} rows={rows} canWrite={canWrite} />
            ) : (
              <p className="text-[13px] text-gray-500">
                Pick a site in the top bar. Content belongs to one, so there is
                nothing here to publish or withhold.
              </p>
            )}
          </Panel>
        </>
      ...
      <Panel title="Endpoints" sub={`Base URL: ${base}`}>
        <dl className="flex flex-col gap-4 text-[12.5px]">
          ...
          <Endpoint
            ...
            path="/site"
            auth="public or content:read"
            what="The site's own profile: brand, portal address, checkout address, support address. What a website should read rather than hardcode."
          />
          <Endpoint
            method="GET"
            path="/catalog"
            auth="catalog:read"
            what="Plans, entitlements and every price. A pricing page rendered from here cannot disagree with checkout."
          />
          <Endpoint
            method="GET"
            path="/orders"
            auth="orders:read"
            what="This site's subscriptions and their status, paged. Never public: it carries customer email addresses."
          />
          <Endpoint
            method="GET"
            path="/me"
            auth="any"
            what="What the presented credential is, what it may do, and which site it speaks for. The endpoint to point at when a key is not working."
          />
        </dl>
...
{`# public, no credential: name the site, since there is no default one
curl ${base}/content -H "X-Site-Key: ${site?.key ?? "your-site"}"
...
curl ${base}/content/homepage-banner \\
  -H "Authorization: Bearer sx_live_..."`}
      ...
      <Note>
        ...
        . This screen only decides who may read them. Keys, verified domains and
        webhook endpoints all live on the site itself, under{" "}
        <Link href="/admin/sites" className="text-[#2B50DC] dark:text-[#5B8DEF]">
          Sites
        </Link>
        .
      </Note>
