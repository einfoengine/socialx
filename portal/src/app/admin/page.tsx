...
export default async function AdminOverview() {
  ...
  /* Every tile below counts one site's work. An overview that summed two
     customers into one number would be worse than no overview: it would look
     like a business result and be an artefact of who happens to be on the
     platform. */
  ...
  const [
    ...
    seeBatches
      ...
      : noRows,
    seeBatches ? applySiteFilter(supabase.from("posts").select("id, status"), filterId) : noRows,
    seePublishing
      ? applySiteFilter(supabase.from("posts").select("id"), filterId).eq("status", "approved")
      : noRows,
    ...
    seeLibrary
      ? /* cross-site: the library is platform IP shared by every site, so a
           broken asset in it is broken for all of them. */
