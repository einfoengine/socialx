...
export async function attachLibraryAsset(
  ...
  /* Confirms the asset exists and is a platform library asset rather than a
  ...
  if (asset.org_id) {
    return { ok: false, error: "That file belongs to a client. The library only uses platform assets." };
  }
...
export async function attachUrlAsset(
  ...
  /* cross-site: library assets belong to the platform, not to a site, so a
     sample image is reused across every site's templates. */
