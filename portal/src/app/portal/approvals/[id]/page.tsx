...
export const metadata: Metadata = { title: "Review batch | Portal" };
...
export default async function BatchReview({
  ...
  /* Who a comment is from, when it is not the client. Their supplier is the site
     that sold them, so the label is that site's wordmark rather than this
     platform's name, which the client has never heard of and should not. */
  const site = await portalSite(session.orgId);
  const authorLabel = site ? wordmarkOf(site) : "team";
  ...
  const [{ data: posts }, { data: revisions }, { data: comments }] = await Promise.all([
    ...
    supabase
      /* cross-site: this is the client portal, not the console. RLS narrows comments
         to the reader's own organization, which is narrower than a site. Unfiltered
         here is wasteful rather than unsafe: it fetches every comment the client may
         see and matches them to posts in memory. Worth a batch filter one day. */
      .from("comments")
