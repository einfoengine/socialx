...
import type { MemberRole, StaffRole } from "@/lib/core/types/db";
...
export async function listAccounts(): Promise<AccountRow[]> {
  ...
  const [{ data: users }, { data: profiles }, { data: roles }, { data: memberships }] =
    await Promise.all([
      ...
      /* cross-site: People lists everyone who can sign in, and staff belong to no
         site at all. Scoping the memberships half would produce a roster where the
         accounts came from every site and their client rows came from one. */
