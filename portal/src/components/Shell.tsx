...
import SiteMark, { type SiteMarkBrand } from "./SiteMark";
 ...
 * Square corners throughout, because globals.css zeroes every radius on purpose
 * and a rounded rail here would be the one place the design language breaks.
 *
 * The wordmark is the one piece that is not fixed. /admin is operator software
 * and wears the platform's own mark; /portal belongs to whichever site sold the
 * signed-in client and wears theirs. A client seeing this platform's brand on
 * their own portal would be seeing their supplier's supplier, which is the thing
 * a white-label product exists not to do. Pass `brand` and it is used; leave it
 * out and the platform mark stands.
 */
  ...
  return (
    <header className="sticky top-0 z-40 flex items-center justify-between border-b border-black/10 dark:border-white/10 bg-white/85 dark:bg-[#0C0C12]/85 backdrop-blur-md px-4 md:px-6 h-[57px]">
      <div className="flex min-w-0 items-center gap-2.5">
        {brand ? <SiteMark brand={brand} className="h-[26px]" /> : <Logo className="h-[26px]" />}
...
export default function Shell({
  ...
  area: string;
  /** The site whose mark this chrome wears. Omitted, the platform's own is used. */
  brand?: SiteMarkBrand | null;
  groups: NavGroup[];
  bottom?: NavItem[];
