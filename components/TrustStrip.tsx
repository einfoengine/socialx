export default function TrustStrip() {
  ...
  const marqueeItemsReverse = [...benefits.slice().reverse(), ...benefits.slice().reverse()];

  return (
    <section
      id="gw-trust-strip"
      className="py-6 overflow-hidden flex flex-col gap-4 relative bg-[#111118] dark:bg-[#050508] border-t border-black/10 dark:border-white/10 transition-colors duration-300"
    >
      ...
      <div className="flex animate-marquee-reverse whitespace-nowrap w-max items-center">
        {marqueeItemsReverse.map((benefit, i) => (
          <div key={i} className="flex items-center mx-8">
            <span className="text-[#2B50DC] mr-4 text-xl leading-none">✦</span>
            <span className="font-grotesk text-[13px] font-semibold text-white/80 uppercase tracking-[1.5px]">
              {benefit}
